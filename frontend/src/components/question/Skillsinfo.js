  ...
  types: [
    ...
    const handleModalButtonClick = () => {
        ...
        if (status === 1) {
          ...
          navigate('/skills1', { state: { frontend,backend, others } });
        } else if (status === 2) {
    ...
    const handleFrontend = (index) => {
        const clickedSkill = skilllist[0].types[index]
        setFrontskills((prevSkills) => [...prevSkills, clickedSkill])
        console.log(frontskills)
      };
      const handleBackend = (index) => {
        const clickedSkill = skilllist[1].types[index]
        setBackskills((prevSkills) => [...prevSkills, clickedSkill])
        console.log(backskills)
      };
      const handleOthers = (index) => {
        const clickedSkill = skilllist[2].types[index]
        setOthers((prevSkills) => [...prevSkills, clickedSkill])
        console.log(others)
      };
